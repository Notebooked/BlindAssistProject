...
while True:
    ...
    if current_time - last_tts_time >= tts_interval:
        if detected_objects:
            ...
            object_dict = {}
            for obj_name, confidence, position, depth in detected_objects:
                key = f"{obj_name}_{position}"
                if key not in object_dict or confidence > object_dict[key][1]:
                    object_dict[key] = (obj_name, confidence, position, depth)
            
            # Create TTS text with position information
            object_descriptions = []
            for obj_name, confidence, position, depth in object_dict.values():
                object_descriptions.append(
                    f"{obj_name} {position}, approximately {depth:.2f} meters away, with {confidence*100:.0f} percent confidence"
                )
